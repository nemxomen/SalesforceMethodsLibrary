...
public class FileBasedDeployAndRetrieve {
	...
	static final double API_VERSION = 37.0;
	...
	static void deployZip(File packaged) throws Exception {
		byte zipBytes[] = readZipFile(packaged);
		...
		System.out.println("The file " + packaged + " was successfully deployed\n");
	}

	/*
	 * Read the zip file contents into a byte array.
	 */
	private static byte[] readZipFile(File packaged) throws Exception {
		byte[] result = null;
		// We assume here that you have a deploy.zip file.
		// See the retrieve sample for how to retrieve a zip file.
		File zipFile = packaged;
	...
	static void retrieveZip(File unpackage) throws Exception {
		RetrieveRequest retrieveRequest = new RetrieveRequest();
		File packages = unpackage;
		// The version in package.xml overrides the version in RetrieveRequest
		retrieveRequest.setApiVersion(API_VERSION);
		setUnpackaged(retrieveRequest,packages);
	...
	private static RetrieveResult waitForRetrieveCompletion(AsyncResult asyncResult) throws Exception {
	...
	private static void setUnpackaged(RetrieveRequest request, File xml) throws Exception {
		// Edit the path, if necessary, if your package.xml file is located elsewhere
		File unpackedManifest = xml;
